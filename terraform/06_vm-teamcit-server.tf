### terraform/06_vm-docker.tf

resource "yandex_compute_instance" "teamcity_server" {
  name        = "teamcity-server-${var.flow}"
  hostname    = "teamcity-server-${var.flow}"
  ...
  metadata = {
    user-data = templatefile("cloud-init/cloud-init-teamcit-server.tpl", {
