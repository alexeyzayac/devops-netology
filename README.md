...
```bash
git checkout -b 16.05_terraform_team_setup-hotfix

cd src-04

docker run --rm --tty -v "$(pwd):/tflint" -w /tflint ghcr.io/terraform-linters/tflint

docker run --rm --tty -v "$(pwd):/checkov" -w /checkov bridgecrew/checkov --download-external-modules true --directory /checkov
```